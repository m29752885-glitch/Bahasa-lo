# bahasalo.py - Mini REPL Interpreter Bahasa Lo (Full Linux Support)
import re
import subprocess

# Tempat nyimpen variabel
variabel = {}
...
def evaluasi_ekspresi(ekspresi):
    """Evaluasi ekspresi sederhana termasuk variabel"""
    for var in variabel:
...
def proses_baris(b):
    ...
    # Semua perintah lain dijalankan di Linux (apt, git, wget, dll.)
    try:
        subprocess.run(b, shell=True, check=True)
    except subprocess.CalledProcessError as e:
        print("Terjadi kesalahan:", e)
    return None

def repl():
    print("Selamat datang di Mini Bahasa Lo (REPL + Semua Linux Command)")
